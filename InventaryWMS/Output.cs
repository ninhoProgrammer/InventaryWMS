...
namespace InventaryWMS
{
    public partial class Output : Form
    {
        ...
        #region Constructores y Métodos de Inicialización
        public Output()
        {
            InitializeComponent();
        }
        ...
        private void Inicialice()
        {
            try
            {
                ...
                comboBoxSerial.AutoCompleteSource = AutoCompleteSource.ListItems;
            }
            catch { }
        }
        #endregion
        ...
        private void buttonLoad_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (formShowQuantity.comboBoxRemission.Text != "")
                {
                    if (selectSQL.existsRemission(formShowQuantity.prefix))
                    {
                        ...
                        switch (gsr)
                        {
                            ...
                            case 3:
                                trueValidacion();
                                buttonRemmisionCancel.Visible = false;
                                labelRemiss.Visible = true;
                                comboBoxShip.Enabled = false;
                                comboBoxBill.Enabled = false;
                                break;
                            case 4:
                                inicilizeForm(false);
                ...
                {
                    MessageBox.Show("No hay remision seleccionada");
                }
            }
            catch { }
            dataProducts.ClearSelection();
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Error: Seleccione qué producto agregar.");
                return;
            }

            // 2. Ejecutar la consulta en segundo plano para no congelar la UI
            DataTable dataTableFromDb;
            try
            {
                dataTableFromDb = GetOrderedData();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Ocurrió un error al obtener datos: {ex.Message}");
                return;
            }

            // 3. Lógica principal basada en el tipo de orden
            if (_order) // Lógica para la orden de cajas (Box)
            {
                ProcessBoxOrder(dataTableFromDb);
            }
            else // Lógica para la orden por cantidad (Quantity)
            {
        ...
        #region Lógica de Negocio y Métodos Auxiliares
        ...
        #region Métodos de Visualización (UI)
        ...
        #region Métodos de Impresión y Reportes
        ...
        private void buttonRemission_Click(object sender, EventArgs e)
        {
            ...
            //reportPath = @"C:\Users\ABG_SISTEMAS\Documents\Documents\GitHub\ReisWMS\ReisWMS\Remision.rdlc";
            ...
            if (resultado == DialogResult.OK)
            {
                ...
                foreach (DataRow row in aux.Rows)
                {
                    if (serachindatatable(compressed, "PROVPARTNUM", row["PROVPARTNUM"].ToString()) == false || serachindatatable(compressed, "LOTE", row["LOTE"].ToString()) == false)
                    {
                        ...
                        newrow["PROVPARTNUM"] = row["PROVPARTNUM"].ToString() + " | " + row["CUSTPARTNUM"].ToString();
                        newrow["CUSTPARTNUM"] = row["CUSTPARTNUM"].ToString();
                        newrow["DESCRIPTION"] = row["DESCRIPTION"].ToString();
                        newrow["SERIAL"] = row["SERIAL"].ToString();
                        newrow["REMISSION_AT"] = row["REMISSION_AT"].ToString();
                        compressed.Rows.Add(newrow);
                    }
                    else
                    {
                        foreach (DataRow row2 in compressed.Rows)
                        {
                            if (row2["PROVPARTNUM"].ToString() == row["PROVPARTNUM"].ToString())
                            {
                                row2["CANTIDAD_ORIGINAL"] = int.Parse(row2["CANTIDAD_ORIGINAL"].ToString()) + int.Parse(row["CANTIDAD_ORIGINAL"].ToString());
                                break;
                            }
                        }
                    }
        ...
        private void addQuetityBox()
        {
            ...
            foreach (DataGridViewRow row in dataProducts.Rows)
            {
                if (row.Cells["QTY"].Value != null && row.Cells["QTY"].Value != DBNull.Value)
                {
                    ...
                    _quantityBox += int.Parse(row.Cells["QTY"].Value.ToString());
                }
            }
        ...
        {
            // Usar String.IsNullOrWhiteSpace para una validación más robusta
            return !string.IsNullOrWhiteSpace(comboBoxPart.Text) ||
                   !string.IsNullOrWhiteSpace(comboBoxLote.Text) ||
                   !string.IsNullOrWhiteSpace(comboBoxSerial.Text);
        }

        private DataTable GetOrderedData()
        {
            createWhere();
            return selectSQL.SearchInOrder(_idClient, where);
        }

        private void ProcessBoxOrder(DataTable dataT)
        {
            if (!int.TryParse(textBoxBox.Text, out int orderedQuantity))
            {
            ...
            {
                MessageBox.Show($"No tiene suficiente material para este embarque; solo tiene: {amountObtained} artículos.");
                return;
            }
        ...
        private void ProcessQuantityOrder(DataTable dataT)
        {
            if (!int.TryParse(textBoxQuantity.Text, out int orderedQuantity))
            {
            ...
            UpdateUIForQuantityOrder();
        }

        private void UpdateUIForBoxOrder()
        {
            if (dataProducts.Rows.Count > 0)
            {
        ...
        private void UpdateUIForQuantityOrder()
        {
            // Aclara la lógica del `if (dataProducts.Rows.Count < 0)` que siempre será falsa
            if (dataProducts.Rows.Count >= 0)
            {
                dataProducts.Columns[0].Visible = false;
                dataProducts.Columns[1].Visible = false;
        ...
        private void erraseListSerial()
        {
            DataRow[] rowsToDelete = allSeries.Select($"Serial = '{comboBoxSerial.Text}'");
            if (rowsToDelete.Length > 0)
            {
                rowsToDelete[0].Delete();
                allSeries.AcceptChanges(); // Confirma la eliminación
            }

            // 2. Refrescar el ComboBox para que se actualice la vista
            // Limpiamos y re-asignamos el DataSource para que la lista se redibuje.
            comboBoxSerial.DataSource = null;
            comboBoxSerial.DataSource = allSeries; // Aquí puedes reasignar la tabla filtrada si quieres
