    ...
    internal class SelectSQL
    {
        ...
        */
        ...
        public void RemissionLoadAComboBox(ComboBox COMBOBOX, int IDCLINTE)
        {
            ...
            {
                _connection.Open();
                SqlCommand cmd = new SqlCommand("SELECT ID_REMISSION_HEADER FROM [dbo].[REMISSION_HEADER] WHERE CLIENT_ID = @IDCLINTE ORDER BY ID_REMISSION_HEADER DESC", _connection);
                cmd.Parameters.AddWithValue("@IDCLINTE", IDCLINTE);
                _dr = cmd.ExecuteReader();
